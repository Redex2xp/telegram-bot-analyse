...
Основная задача — преобразование запроса на естественном языке в SQL — решается с помощью модели `google/gemini-2.5-flash-lite`. Ключевым элементом является **промпт**, который подается модели.
